...
const isActive=cond.mode==='show'?result:!result;if(isActive){activateField(cond.id,cond.disableOnly)}else{values[cond.id]='';deactivateField(cond.id,cond.disableOnly)}}}
...
$form.addClass('dce-form-has-conditions');const lang=new expressionLanguage.ExpressionLanguage();fieldIds=JSON.parse(fieldIds);field_conditions=JSON.parse(field_conditions??'[]');submit_conditions=JSON.parse(submit_conditions??'[]');const fields=getAllFields($form,fieldIds);let onFormChange=getOnFormChange({fields:fields,field_conditions:field_conditions,submit_conditions:submit_conditions,$form:$form,lang:lang,});onFormChange();let $all=$form.find('input, select');$all.on('change',onFormChange)}
