...
let getFieldValue=(form,id)=>{let data=new FormData(form);if(data.has(`form_fields[${id}]`)){return data.get(`form_fields[${id}]`)}else if(data.has`form_fields[$id][]`){return data.getAll(key)}
return""}
let makeGetFieldFunction=(form)=>{return(id,toFloat=!1)=>{let val=getFieldValue(form,id);return toFloat?parseFloat(val):val}}
...
jQuery(window).on('elementor/frontend/init',function(){elementorFrontend.hooks.addAction('frontend/element_ready/form.default',initializeAllJsFieldFields)})