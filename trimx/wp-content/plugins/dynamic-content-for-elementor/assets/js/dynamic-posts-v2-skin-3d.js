...
var Widget_DCE_Dynamicposts_3d_Handler = function ($scope, $) {
    ...
    var scene3d_wrapper = $scope.find('.dce-3d-wrapper');
    var scene3d_target = '.dce-post-image';
    var postsItems = scene3d.find('.dce-post-item');
    ...
    // default values ----------
    var panelWidth = Number(elementSettings[DCE_dynposts_skinPrefix+'size_plane_3d']) || 320;
    var panelSpace = 400;
    var distanza = 1000;
    //
    ...
    var camDefaultZ = raggio+distanza;
    var camDefaultY = 300;

    var eneableblur = Boolean(elementSettings[DCE_dynposts_skinPrefix+'blur_depth_3d']) || false
    var positionType = elementSettings[DCE_dynposts_skinPrefix+'type_3d']  || 'circle';
    ...
    function init() {
        ...
        for ( var i = 0; i < objects.length; i ++ ) {
            ...
            targets.circle.push( calcolorotazione(i,raggio) );
            targets1.circle.push( calcolorotazione(i,raggio+distanza) );
        ...
        if(elementSettings[DCE_dynposts_skinPrefix+'mousewheel_3d'])
        ...
        if(!isAnimating){
            ...
            else if(delta > 1)
            {
                if(positionType == 'circle'){
                    ...
                    currentIndex = currentIndex < elementsQuantity-1 ? currentIndex+1 : elementsQuantity-1;
                }
                centerItem( currentIndex, 1000 );
    ...
    function camReset(){
          ...
          for ( var i = 0; i < objects.length; i ++ ) {
            ...
            if(eneableblur) sfocaoggetto(objects[i],true);
          }
    ...
    function render() {
        ...
        for ( var i = 0; i < objects.length; i ++ ) {
            ...
            if(!is3dEnabled){
                if(eneableblur) sfocaoggetto(object);
            }
    ...
    function sfocaoggetto(object,animated = false){
