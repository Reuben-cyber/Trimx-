...
class DCE_Extension_Form_PayPal extends \ElementorPro\Modules\Forms\Fields\Field_Base {
	...
	public function render( $item, $item_index, $form ) {
		...
		$form->add_render_attribute( 'paypal-buttons' . $item_index,
									 'data-name', $name );
		$form->add_render_attribute( 'paypal-buttons' . $item_index,
									 'data-currency', get_option( 'dce_paypal_api_currency', 'USD' ) );
		if ( $item['dce_form_paypal_name_from_field'] === 'yes' ) {
			$form->add_render_attribute( 'paypal-buttons' . $item_index,
										 'data-name-field-id', $item['dce_form_paypal_name_field_id'] );
		} else {
			$form->add_render_attribute( 'paypal-buttons' . $item_index,
										 'data-name', $name );
		}
		...
		if ( \Elementor\Plugin::$instance->editor->is_edit_mode()
			...
			$field_controls = [
				'dce_form_paypal_disable_validation' => [
					...
					'description' => __( 'By switching on this setting you disable validation of the transaction amount. There will still be validation to make sure a valid transaction has been sent. This is useful for payments that can have arbitrary amounts, when passed from another form with a request/get parameter', 'dynamic-content-for-elementor' ),
					'type' => \Elementor\Controls_Manager::SWITCHER,
					'default' => 'no',
					'return_value' => 'yes',
					'tab' => 'content',
					...
					'condition' => [
						'field_type' => $this->get_type(),
						'dce_form_paypal_value_from_field!' => 'yes',
					],
				],
				'dce_form_paypal_name_from_field' => [
					'name' => 'dce_form_paypal_name_from_field',
					'label' => __( 'Get item name dynamically from another field in the form', 'dynamic-content-for-elementor' ),
					'label_block' => true,
					'type' => \Elementor\Controls_Manager::SWITCHER,
					'tab' => 'content',
					'default' => 'no',
				...
				'dce_form_paypal_name_field_id' => [
					'name' => 'dce_form_paypal_name_field_id',
					'label' => __( 'Name Field ID', 'dynamic-content-for-elementor' ),
					'label_block' => true,
					...
					'condition' => [
						'field_type' => $this->get_type(),
						'dce_form_paypal_name_from_field' => 'yes',
					],
				],
				...
				'dce_form_paypal_value_from_field' => [
					'name' => 'dce_form_paypal_value_from_field',
					'label' => __( 'Get item value dynamically from another field in the form', 'dynamic-content-for-elementor' ),
					'label_block' => true,
					'type' => \Elementor\Controls_Manager::SWITCHER,
					'tab' => 'content',
					'default' => 'no',
				...
				'dce_form_paypal_value_field_id' => [
					'name' => 'dce_form_paypal_value_field_id',
					'label' => __( 'Value Field ID', 'dynamic-content-for-elementor' ),
					'label_block' => true,
					'type' => \Elementor\Controls_Manager::TEXT,
					'default' => '',
					'tab' => 'content',
	...
	public function process_field( $field, Classes\Form_Record $record, Classes\Ajax_Handler $ajax_handler ) {
		...
		try {
			...
			if ( 'yes' !== $field_settings['dce_form_paypal_disable_validation'] ||
				 'yes' === $item ['dce_form_paypal_value_from_field'] ) {
