...
class DCE_Extension_Form_Email extends \ElementorPro\Modules\Forms\Classes\Action_Base {
	...
	public function __construct() {
		self::add_dce_email_template_type(); // Add specific Template Type
	}
	/**
	 * Get Name
	 *
	...
	public function register_settings_section( $widget ) {
		...
		if ( \Elementor\Plugin::$instance->editor->is_edit_mode()
			...
			$widget->add_control(
				'admin_notice', [
					...
					'raw' => '<div class="elementor-panel-alert elementor-panel-alert-warning">' . __( 'You will need administrator capabilities to edit these settings.', 'dynamic-content-for-elementor' ) . '</div>',
				]
			);
		...
		$repeater_fields->add_control(
				...
				[
					...
					'default' => 'yes',
					'description' => __( 'You can temporary disable and reactivate it next time without deleting settings ', 'dynamic-content-for-elementor' ),
					'separator' => 'after',
				]
		);
		$repeater_fields->add_control(
				'dce_form_email_condition_field', [
					...
					'description' => __( 'Type here the form field ID to check, or leave it empty to always execute this action', 'dynamic-content-for-elementor' ),
				]
		);
		$repeater_fields->add_control(
		...
		$repeater_fields->add_control(
				'dce_form_email_condition_value', [
					...
					'condition' => [
						'dce_form_email_condition_field!' => '',
						'dce_form_email_condition_status' => [ 'lt', 'gt', 'equal', 'contain' ],
					],
				]
		);
		...
		$repeater_fields->add_control(
				...
				[
					'label' => __( 'Subject', 'dynamic-content-for-elementor' ),
					'type' => Controls_Manager::TEXT,
		...
		$repeater_fields->add_control(
				'dce_form_email_to',
				[
					'label' => __( 'To', 'dynamic-content-for-elementor' ),
					'type' => Controls_Manager::TEXT,
					'default' => get_option( 'admin_email' ),
					'placeholder' => get_option( 'admin_email' ),
					'label_block' => true,
					'title' => __( 'Separate emails with commas', 'dynamic-content-for-elementor' ),
					'render_type' => 'none',
					'separator' => 'before',
				]
		);
		...
		$repeater_fields->add_control(
				'dce_form_email_from',
				[
					'label' => __( 'From Email', 'dynamic-content-for-elementor' ),
					'type' => Controls_Manager::TEXT,
		...
		$repeater_fields->add_control(
				...
				[
					'label' => __( 'From Name', 'dynamic-content-for-elementor' ),
					'type' => Controls_Manager::TEXT,
		...
		$repeater_fields->add_control(
				...
				[
					'label' => __( 'Reply-To', 'dynamic-content-for-elementor' ),
					'type' => Controls_Manager::TEXT,
					'render_type' => 'none',
				]
		);
		...
		$repeater_fields->add_control(
				...
				[
					'label' => __( 'Cc', 'dynamic-content-for-elementor' ),
					'type' => Controls_Manager::TEXT,
					'default' => '',
					'title' => __( 'Separate emails with commas', 'dynamic-content-for-elementor' ),
					'render_type' => 'none',
		...
		$repeater_fields->add_control(
				...
				[
					'label' => __( 'Bcc', 'dynamic-content-for-elementor' ),
					'type' => Controls_Manager::TEXT,
					'default' => '',
					'title' => __( 'Separate emails with commas', 'dynamic-content-for-elementor' ),
					'render_type' => 'none',
		...
		$repeater_fields->add_control(
				...
				[
					'label' => __( 'Send As', 'dynamic-content-for-elementor' ),
					'type' => Controls_Manager::SELECT,
					'default' => 'html',
					'render_type' => 'none',
					'options' => [
						'html' => __( 'HTML', 'dynamic-content-for-elementor' ),
						'plain' => __( 'Plain', 'dynamic-content-for-elementor' ),
					],
					'separator' => 'before',
				]
		);
		...
		$repeater_fields->add_control(
				...
				[
					'label' => __( 'Message', 'dynamic-content-for-elementor' ),
					'type' => Controls_Manager::WYSIWYG,
					'default' => '[all-fields]',
					'placeholder' => '[all-fields]',
					'description' => sprintf( __( 'By default, all form fields are sent via %s shortcode. To customize sent fields, copy the shortcode that appears inside each field and paste it above.', 'dynamic-content-for-elementor' ), '<code>[all-fields]</code>' ),
					'label_block' => true,
					'render_type' => 'none',
					'condition' => [
		...
		$repeater_fields->add_control(
				...
				[
					...
					'description' => __( 'Use an Elementor Template as body for this Email.', 'dynamic-content-for-elementor' ),
					'condition' => [
						'dce_form_email_content_type' => 'html',
		...
		$repeater_fields->add_control(
				'dce_form_email_attachments_delete',
				[
					'label' => __( 'Delete Files after Emails are sent', 'dynamic-content-for-elementor' ),
					'type' => \Elementor\Controls_Manager::SWITCHER,
					'description' => __( 'Remove all uploaded Files from Server after Email is sent with the Files as Attachments', 'dynamic-content-for-elementor' ),
					'condition' => [
						'dce_form_email_attachments!' => '',
					],
				]
		...
		$repeater_fields->add_control(
			...
			[
				'label' => __( 'Delete PDF attachments after Emails are sent', 'dynamic-content-for-elementor' ),
				'type' => \Elementor\Controls_Manager::SWITCHER,
				'default' => 'no',
				'description' => __( 'Remove all attached PDF files from Server after Email is sent.', 'dynamic-content-for-elementor' ),
			]
		);
		...
		$widget->add_control(
				'dce_form_email_help', [
					...
					'raw' => '<div id="elementor-panel__editor__help" class="p-0"><a id="elementor-panel__editor__help__link" href="' . $this->doc_url . '" target="_blank">' . __( 'Need Help', 'dynamic-content-for-elementor' ) . ' <i class="eicon-help-o"></i></a></div>',
					'separator' => 'before',
				]
		);
			...
			if ( $amail['dce_form_email_enable'] ) {
				...
				if ( $condition_satisfy ) {
					...
					if ( ! $email_sent ) {
						...
						if ( $amail['dce_form_pdf_attachments_delete'] ) {
							$remove_pdf_files = true;
						}
					}
				}
			}
