				...
				foreach ( $settings['form_fields'] as $key => $afield ) {
					if ( $afield['field_type'] == 'text' ) {
						...
						autocomplete.setFields(['address_component']);
							<?php if ( ! empty( $afield['field_address_restrict_country'] ) ) { ?>
	...
	public static function _add_to_form( Controls_Stack $element, $control_id, $control_data, $options = [] ) {
		...
		if ( $element->get_name() == 'form' && $control_id == 'form_fields' ) {
			...
			if ( ! get_option( 'dce_google_maps_api' ) ) {
				$control_data['fields']['field_address_api_notice'] = array(
					'name' => 'field_address_api_notice',
					'type' => Controls_Manager::RAW_HTML,
					'raw' => __( 'In order to use Address Autocomplete you should set Google Maps API, with Geocoding API enabled, on APIs section', 'dynamic-content-for-elementor' ),
			...
			$control_data['fields']['field_address'] = array(
				'name' => 'field_address',
				'label' => __( 'Address Autocomplete', 'dynamic-content-for-elementor' ),
			...
			$control_data['fields']['field_address_restrict_country'] = array(
				'name' => 'field_address_restrict_country',
				'label' => __( 'Restrict Country', 'dynamic-content-for-elementor' ),
				'type' => Controls_Manager::SELECT2,
				'options' => Helper::get_iso_3166_1_alpha_2(),
				'multiple' => true,
				'conditions' => [
					'terms' => [
						[
