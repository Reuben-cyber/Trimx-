...
class DCE_Extension_Form_Telegram extends \ElementorPro\Modules\Forms\Classes\Action_Base {
	...
	public function register_settings_section( $widget ) {
		...
		$repeater_fields->add_control(
			'dce_form_telegram_token',
			[
				'label' => __( 'Telegram authorization token', 'dynamic-content-for-elementor' ),
				'type' => Controls_Manager::TEXT,
				'render_type' => 'none',
				'separator' => 'before',
			]
		...
		$repeater_fields->add_control(
			'dce_form_telegram_chat_id',
			[
				'label' => __( 'Chat ID', 'dynamic-content-for-elementor' ),
				'type' => Controls_Manager::TEXT,
				'render_type' => 'none',
			]
		);
		...
		$repeater_fields->add_control(
			...
			[
				...
				'default' => __( 'New message from', 'dynamic-content-for-elementor' ) . ' [form:name]:[form:message]',
				'placeholder' => __( 'New message from', 'dynamic-content-for-elementor' ) . ' [form:name]:[form:message]',
				'label_block' => true,
				'render_type' => 'none',
			]
		);
		...
		[
			...
			'title_field' => '{{{ dce_form_telegram_content }}}',
			...
			if ( $telegram_message['dce_form_telegram_enable'] && $telegram_message['dce_form_telegram_token'] && $telegram_message['dce_form_telegram_chat_id'] && $telegram_message['dce_form_telegram_content'] ) {
				...
				if ( $condition_satisfy ) {
					...
					$response = $telegram->sendMessage(
						$settings_raw['dce_form_telegram_repeater'][ $mkey ]['dce_form_telegram_chat_id'],
						$dce_form_telegram_content
					);
