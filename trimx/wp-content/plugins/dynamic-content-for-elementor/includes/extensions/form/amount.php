		...
		if ( $element->get_name() == 'form' ) {
			...
			if ( $control_id == 'form_fields' ) {
				...
				if ( $control_id == 'form_fields' ) {
					...
					$control_data['fields']['dce_amount_text_before'] = array(
						'name' => 'dce_amount_text_before',
					...
					$control_data['fields']['dce_amount_text_after'] = array(
						'name' => 'dce_amount_text_after',
				...
				if ( ! empty( $settings['form_fields'] ) ) {
					foreach ( $settings['form_fields'] as $key => $afield ) {
						if ( $afield['field_type'] == 'amount' ) {
							...
							$text_before = sanitize_text_field( $afield['dce_amount_text_before'] );
							$text_after = sanitize_text_field( $afield['dce_amount_text_after'] );
							$fields_name = array();
							...
							foreach ( $pieces as $apiece ) {
								...
								if ( count( $tmp ) > 1 ) {
									...
									if ( isset( $form_fields[ $field_name ] ) ) {
										...
										if ( in_array( $form_fields[ $field_name ]['field_type'], [ 'select', 'dynamic_select' ], true ) ) {
							...
							if ( ! empty( $fields_name ) ) {
								?>
							jQuery('<?php echo implode( ', ', $fields_name ); ?>').on('input', function () {
								setTimeout(function () {
									let amountResult = ( <?php echo $js_expression; ?> );
									jQuery('.elementor-element-<?php echo $widget->get_id(); ?> #dce-form-field-<?php echo $afield['custom_id']; ?>').val(amountResult);
									jQuery('.elementor-element-<?php echo $widget->get_id(); ?> #form-field-<?php echo $afield['custom_id']; ?>').val('<?php echo $text_before; ?>' + amountResult + '<?php echo $text_after; ?>');
									jQuery('.elementor-element-<?php echo $widget->get_id(); ?> #form-field-<?php echo $afield['custom_id']; ?>').trigger('input');
								}, 100);
							});
