...
class DCE_Extension_Visibility extends DCE_Extension_Prototype {
	...
	public static function compare_options() {
		return [
			'not' => [
				'title' => __( 'Not set or empty', 'dynamic-content-for-elementor' ),
				'icon' => 'fa fa-circle-o',
			],
			'isset' => [
	...
	private function add_controls( $element, $args ) {
		...
		if ( $section == 'dynamic_tag' ) {
			...
			$element->add_control(
				...
				[
					'label' => __( 'Dynamic Tag', 'dynamic-content-for-elementor' ),
					'type' => Controls_Manager::MEDIA,
					'dynamic' => [
						'active' => true,
						'categories' => $refl->getConstants(),
					],
					'placeholder' => __( 'Select condition field', 'dynamic-content-for-elementor' ),
				]
			);
			$element->add_control(
		...
		if ( $section == 'post' ) {
			...
			$element->add_control(
				...
				[
					...
					'query_type' => 'posts',
					'object_type' => 'type',
					'description' => __( 'Visible if current post is one of these Post Types', 'dynamic-content-for-elementor' ),
				]
			);
			...
			$element->add_control(
				...
				[
					...
					'label_block' => true,
					'query_type' => 'posts',
					'description' => __( 'Visible if current post is one of these Page/Posts', 'dynamic-content-for-elementor' ),
					'multiple' => true,
					'separator' => 'before',
				]
			);
			...
			foreach ( $taxonomies as $tkey => $atax ) {
				if ( $tkey ) {
					...
					[
						...
						'query_type' => 'terms',
						'object_type' => $tkey,
						'description' => __( 'Visible if current post is related to these terms', 'dynamic-content-for-elementor' ),
						'multiple' => true,
						'condition' => [
