...
class DCE_Extension_Unwrap extends DCE_Extension_Prototype {
	...
	private function add_controls( $element, $args ) {
		...
		$element->add_control(
				'dce_unwrap', [
					'label' => __( 'Unwrap Element', 'dynamic-content-for-elementor' ),
		...
		$element->add_control(
				'dce_unwrap_warning',
				[
					'raw' => '<strong>' . __( 'Please note!', 'dynamic-content-for-elementor' ) . '</strong> ' . __( 'Removing the wrappers could disable all styles and javascript features on the element', 'dynamic-content-for-elementor' )
					. '<br><a target="_blank" href="https://help.dynamic.ooo/en/articles/4952484-unwrap">' . __( 'Read how and why you should use it', 'dynamic-content-for-elementor' ) . '</a>',
		...
		$element->add_control(
				'dce_unwrap_style', [
					'label' => __( 'Try to force Element Style', 'dynamic-content-for-elementor' ),
					'type' => Controls_Manager::SWITCHER,
					'default' => 'yes',
					'condition' => [
						'dce_unwrap!' => '',
		...
		$element->add_control(
				'dce_unwrap_strip', [
					'label' => __( 'Strip selected Tags', 'dynamic-content-for-elementor' ),
					'type' => Controls_Manager::TEXT,
					'placeholder' => 'div, p, span',
					'condition' => [
						'dce_unwrap!' => '',
					],
				]
	...
	protected function add_actions() {
		...
		add_action('elementor/documents/register_controls', function( $element ) {
			...
			$element->start_controls_section(
					...
					[
						...
						'label' => __( 'Unwrap', 'dynamic-content-for-elementor' ),
					]
			);
		...
		add_action( 'elementor/frontend/the_content', array( $this, 'end_element' ), 10, 2 ); // template end
			...
			if ( ! empty( $settings['dce_unwrap'] ) ) {
				...
				if ( ! empty( $content ) ) {
					...
					if ( $settings['dce_unwrap_style'] ) {
						...
						// output
						$content_unwrapped = $cssToInlineStyles->convert(
								$content_unwrapped,
								$css
						);
