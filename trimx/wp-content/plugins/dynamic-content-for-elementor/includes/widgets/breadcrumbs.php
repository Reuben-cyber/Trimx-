...
class DCE_Widget_Breadcrumbs extends DCE_Widget_Prototype {
	...
	protected function _register_controls() {
		...
		if ( ! function_exists( 'yoast_breadcrumb' ) || ! $this->is_yoast_breadcrumbs() ) {
			...
			$this->add_control(
				'home-text', [
					'label' => __( 'Custom Home Text', 'dynamic-content-for-elementor' ),
					'type' => Controls_Manager::TEXT,
					'default' => __( 'Homepage', 'dynamic-content-for-elementor' ),
					'condition' => [
		...
		$this->add_control(
			'final_color', [
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .dce-breadcrumbs .bread-current, {{WRAPPER}} .dce-breadcrumbs .breadcrumb_last' => 'color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_group_control(
			Group_Control_Typography::get_type(), [
				...
				'selector' => '{{WRAPPER}} .dce-breadcrumbs .bread-current, {{WRAPPER}} .dce-breadcrumbs .breadcrumb_last',
			]
		);
		...
		$this->add_control(
			'separator_color', [
				...
				'selectors' => [
					'{{WRAPPER}} .dce-breadcrumbs .dce-separator, .dce-breadcrumbs span' => 'color: {{VALUE}};',
				],
	...
	private function is_yoast_breadcrumbs() {
		...
		if ( ! $breadcrumbs_yoast ) {
			...
			$breadcrumbs_yoast = true === ( ! empty( $options_yoast['breadcrumbs-enable'] ) && $options_yoast['breadcrumbs-enable'] );
		}
