...
class DCE_Widget_Relationship extends DCE_Widget_Prototype {
	...
	protected function _register_controls() {
		...
		$this->add_control(
			'acf_relation_taxonomy', [
				'label' => __( 'Retrieve the current term from this taxonomy', 'dynamic-content-for-elementor' ),
				'type' => Controls_Manager::SELECT,
				'options' => get_taxonomies( array( 'public' => true ) ),
				'default' => 'category',
				'label_block' => true,
				'condition' => [
					'acf_relation_from' => 'current_term',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'condition' => [
					'acf_relation_from' => 'current_post',
				],
			]
		);
		...
		$this->add_control(
			'acf_relation_draft',
			[
				'label' => __( 'Ignore drafts', 'dynamic-content-for-elementor' ),
				'type' => Controls_Manager::SWITCHER,
				'default' => 'yes',
				'separator' => 'before',
			]
		...
		$this->add_control(
			'acf_relation_orderby',
			[
				'label' => __( 'Order By', 'dynamic-content-for-elementor' ),
				'type' => Controls_Manager::SELECT,
				'options' => array_merge( [ 'post__in' => __( 'Same order of ACF Relationship', 'dynamic-content-for-elementor' ) ], Helper::get_post_orderby_options() ),
				'default' => 'post__in',
			]
		);
		$this->add_control(
			'acf_relation_metakey',
			[
				...
				'object_type' => 'post',
				'condition' => [
					'acf_relation_orderby' => [ 'meta_value_date', 'meta_value_num', 'meta_value' ],
				],
			]
		);
		$this->add_control(
		...
		$this->add_control(
			...
			[
				...
				'options' => [
					...
					'text' => [
						'title' => __( 'Text editor', 'dynamic-content-for-elementor' ),
		...
		$this->add_control(
			...
			[
				...
				'default' => '<h4>[post:title]</h4>[post:thumb]<p>[post:excerpt]</p><a class="btn btn-primary" href="[post:permalink]">' . __( 'Read more', 'dynamic-content-for-elementor' ) . '</a>',
		...
		$this->add_control(
			'acf_relation_close',
			[
				'label' => __( 'Closed by default', 'dynamic-content-for-elementor' ),
				'type' => Controls_Manager::SWITCHER,
				'condition' => [
		...
		$this->add_responsive_control(
			'acf_relation_col_align',
			[
				'label' => __( 'Columns Align', 'dynamic-content-for-elementor' ),
				'type' => Controls_Manager::CHOOSE,
				'options' => [
					'left' => [
				...
				'selectors' => [
					'{{WRAPPER}} .grid-page' => 'justify-content: {{VALUE}};',
				],
				'condition' => [
					'acf_relation_format' => 'grid',
				],
		...
		$this->add_responsive_control(
			...
			[
				...
				'default' => '',
				'selectors' => [
					'{{WRAPPER}} .elementor-heading-title, {{WRAPPER}} .dce-acf-relationship-natural' => 'text-align: {{VALUE}};',
				],
			]
		);
		$this->add_control(
	...
	protected function render() {
		...
		if ( $settings['acf_relation_invert'] ) {
			...
			if ( $settings['acf_relation_from'] == 'current_post' ) {
				$rel_posts = get_post_meta( get_the_ID(), $settings['acf_relation_field'], true );
				...
				$rel_posts = get_user_meta( get_current_user_id(), $settings['acf_relation_field'], true );
			} elseif ( $settings['acf_relation_from'] == 'current_term' ) {
				$terms = get_the_terms( get_the_ID(), $settings['acf_relation_taxonomy'] );
				$rel_posts = get_term_meta( $terms[0]->term_id, $settings['acf_relation_field'], true );
		...
		if( 'attachment' === get_post_type( $rel_posts[0] ) ) {
			$args = [
				...
				'post__in' => $rel_posts,
				'post_status' => 'inherit',
				'orderby' => $acf_relation_orderby,
				'order' => $order,
				'meta_key' => $settings['acf_relation_metakey'],
			];
		} else {
			$args = [
				'post_type' => 'any',
		...
		foreach ( $relationship_results as $post ) {
						...
						if ( $settings['acf_relation_format'] == 'accordion' && $settings['acf_relation_render'] != 'title' ) {
							?>
								  ...
								  <div class="dce-accordion-item">
									  <a class="dce-view-item" href="#dce-acf-relational-post-<?php echo $this->get_id() . '-' . get_the_ID(); ?>" onclick="if (!jQuery(jQuery(this).attr('href')).is(':visible')) {
			...
			if ( $settings['acf_relation_render'] == 'template' && $settings['acf_relation_template'] ) {
				...
				echo Tokens::do_tokens( $settings['acf_relation_text'] );
			} else {
