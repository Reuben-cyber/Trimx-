...
class DCE_Widget_SvgDistortion extends DCE_Widget_Prototype {
	...
	protected function _register_controls_content() {
		$this->start_controls_section(
			'section_distortion', [
				'label' => __( 'Distortion', 'dynamic-content-for-elementor' ),
			]
		);
		$this->add_control(
			'svg_trigger', [
		...
		$this->add_control(
		  'base_image',
		  [
			  'label' => __( 'The base Image', 'dynamic-content-for-elementor' ),
		...
		$this->add_control(
		  'displacement_image',
		  [
			  'label' => __( 'Displacement Map Image', 'dynamic-content-for-elementor' ),
			  'type' => Controls_Manager::MEDIA,
			  'dynamic' => [
				  'active' => true,
			  ],
		...
		$this->add_control(
			'displacementmap_heading',
			[
				'label' => __( 'DisplacementMap', 'dynamic-content-for-elementor' ),
				'type' => Controls_Manager::HEADING,
				'separator' => 'before',
			]
		);
		...
		$this->add_control(
			'disp_factor_to', [
				'label' => __( 'Depth Factor: TO', 'dynamic-content-for-elementor' ),
				'type' => Controls_Manager::SLIDER,
				'default' => [
					'size' => '0',
					'unit' => 'px',
		...
		$this->add_control(
			'disp_scale_to', [
				'label' => __( 'Scale TO (%)', 'dynamic-content-for-elementor' ),
				'type' => Controls_Manager::SLIDER,
				'default' => [
					'size' => '100',
				],
		...
		'random_animation',
		  [
			  'label' => __( 'Random animation', 'dynamic-content-for-elementor' ),
		...
		$this->add_control(
			'random_animation_range', [
				'label' => __( 'Range of value', 'dynamic-content-for-elementor' ),
				'type' => Controls_Manager::SLIDER,
				'frontend_available' => true,
				'default' => [
					'size' => '100',
		...
		$this->add_control(
			'preserveMode', [
				'label' => __( 'Ratio mode ', 'dynamic-content-for-elementor' ),
				'type' => Controls_Manager::SELECT,
				'options' => [
					'slice' => __( 'Crop', 'dynamic-content-for-elementor' ),
					'meet' => __( 'Keep', 'dynamic-content-for-elementor' ),
				],
				'default' => 'slice',
				'condition' => [
					'preserveAR' => 'yes',
		...
		'svg_align',
			[
				...
				'default' => 'left',
				'selectors' => [
					'{{WRAPPER}} .dce_distortion-wrapper' => 'text-align: {{VALUE}};',
				],
			]
		);
		$this->end_controls_section();
		...
		[
			...
			'description' => __( 'Use distortion only for appling to other page elements. With this Option activated, the svg element will not be displayed.', 'dynamic-content-for-elementor' ),
			'type' => Controls_Manager::SWITCHER,
			'default' => '',
	...
	protected function render() {
		...
		$this->add_render_attribute( 'svgdistortion', [
			'class' => 'dce_distortion',
			'data-coef' => 0.5,
			'data-dispimage' => $dispimage_url,
		] );
		...
		<div class="dce_distortion-wrapper">
		  <div <?php echo $this->get_render_attribute_string( 'svgdistortion' ); ?>>
			<svg  id="dce-svg-<?php echo $widgetId; ?>" data-run="<?php echo $data_run; ?>" class="dce-svg-distortion" xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" viewBox="0 0 <?php echo $viewBoxW; ?> <?php echo $viewBoxH; ?>">
			  <defs>
				<filter id="distortion-filter-<?php echo $widgetId; ?>" x="0" y="0" width="100%" height="100%" filterUnits="objectBoundingBox" primitiveUnits="userSpaceOnUse" color-interpolation-filters="sRGB">
				  <feImage xlink:href="<?php echo $dispimage_url; ?>" id="displacement-image" x="<?php echo $pos_factor; ?>%" y="<?php echo $pos_factor; ?>%" width="<?php echo $scale_factor; ?>%" height="<?php echo $scale_factor; ?>%" preserveAspectRatio="<?php echo $preserveAR; ?>" result="distortionImage" />
			...
			<g id="item-distortion">
			  <image id="img-distorted"
				   preserveAspectRatio="<?php echo $preserveAR; ?>"
				   width="100%"
				   height="100%"
			...
			<style>
			<?php if ( $settings['image_max_width']['size'] && $settings['image_max_width']['size'] > 0 ) { ?>
				#dce-svg-<?php echo $widgetId; ?>{
					max-width: <?php echo $settings['image_max_width']['size']; ?>px;
				}
			<?php } ?>

			<?php if ( $settings['distortion_output'] && $settings['id_svg_class'] != '' ) { ?>
				...
				.<?php echo $id_svg_class; ?> .elementor-button{
					-webkit-filter: url(#distortion-filter-<?php echo $widgetId; ?>);
					filter: url(#distortion-filter-<?php echo $widgetId; ?>);
