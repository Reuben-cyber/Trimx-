...
class DCE_Widget_SvgImagemask extends DCE_Widget_Prototype {
	...
	protected function _register_controls_content() {
		$this->start_controls_section(
			'section_imagemask', [
				'label' => __( 'Imagemask', 'dynamic-content-for-elementor' ),
			]
		);
		$this->add_control(
		  'mask_image',
		...
		$this->add_control(
			'mask_shape_type', [
				...
				'options' => [
					...
					'ring' => __( 'Ring', 'dynamic-content-for-elementor' ),
					'triangle' => __( 'Triangle', 'dynamic-content-for-elementor' ),
					'circle' => __( 'Circle', 'dynamic-content-for-elementor' ),
					'hexagon' => __( 'Hexagon', 'dynamic-content-for-elementor' ),
					'pentagonal' => __( 'pentagon', 'dynamic-content-for-elementor' ),
					'rhombus' => __( 'Rhombus', 'dynamic-content-for-elementor' ),
					'star' => __( 'Star', 'dynamic-content-for-elementor' ),
					'heart' => __( 'Heart', 'dynamic-content-for-elementor' ),
					//'text' => __('Text', 'dynamic-content-for-elementor'),
					'custom_path' => __( 'Custom path', 'dynamic-content-for-elementor' ),
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::HEADING,
				'condition' => [
					'mask_shape_type' => [ 'ring', 'custom_path' ],
				],
			]
		);
		$this->add_control(
		  'shape_numbers',
		  [
			  'label'   => __( 'Custom Path (Numbers)', 'dynamic-content-for-elementor' ),
			  'type'    => Controls_Manager::TEXTAREA,
			  'default' => 'M568,568H0l172.5-284L0,0h568L395.5,287L568,568z',
			  'condition' => [
				  'mask_shape_type' => 'custom_path',
			  ],
		  ]
		);
		$this->add_control(
			'radius_inner_circle',
			[
				'label' => __( 'Radius of the inner circle', 'dynamic-content-for-elementor' ),
		...
		[
			...
			'description' => __( 'Use masking only  for application on other page elements. Activating this option the svg element will not be displayed.', 'dynamic-content-for-elementor' ),
	...
	protected function render() {
	   ...
	   <svg  id="dce-svg-<?php echo $widgetId; ?>" class="dce-svg-imagemask" xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" viewBox="0 0 <?php echo $viewBoxW; ?> <?php echo $viewBoxH; ?>" preserveAspectRatio="xMidYMid slice">

			<defs>
				<mask id="image-mask-<?php echo $widgetId; ?>"maskunits="userSpaceOnUse" maskcontentunits="userSpaceOnUse" preserveAspectRatio="xMidYMid slice">
				   <?php // preserveAspectRatio="none" preserveAspectRatio="xMidYMid slice"
						//maskUnits="[objectBoundingBox | userSpaceOnUse]" ... maskUnits="userSpaceOnUse" maskContentUnits="userSpaceOnUse"
					// https://codepen.io/tutsplus/pen/zREZGe
					if ( $settings['mask_shape_type'] == 'image' ) {
						?>
					<image xlink:href="<?php echo $image_masking_url; ?>"
				   width="100%" height="100%"></image>
						<?php
					} elseif ( $settings['mask_shape_type'] == 'ring' ) {
						?>
					<circle id="outer" cx="50%" cy="50%" r="50%" fill="white"/>
					<circle id="inner" cx="50%" cy="50%" r="<?php echo $settings['radius_inner_circle']['size']; ?>%"/>
						<?php
		...
		c68.4-68.4,179.2-68.4,247.5,0c68.4,68.4,68.4,179.2,0,247.5L299,546.5z" fill="white"/>
					...
					<text id="outer" fill="white"/>>Lorem ipsum</text>
						...
						?>
					<path id="outer" d="<?php echo $settings['shape_numbers']; ?>" fill="white"/>
						<?php
					}
					?>
				</mask>
			</defs>
			<?php if ( ! $settings['mask_output'] ) { ?>
			<image width="100%" height="100%" preserveAspectRatio="xMidYMid slice" xlink:href="<?php echo $image_url; ?>" mask="url(#image-mask-<?php echo $widgetId; ?>)"></image>
			<?php } ?>
			<style>
