	...
	protected function _register_controls() {
			...
			if ( $tkey ) {
				$this->add_control(
						'prent_term_' . $tkey, [
							...
							'type' => Controls_Manager::SELECT,
							'options' => [ 'my_parent' => __( 'My parent', 'dynamic-content-for-elementor' ) ] + Helper::get_parentterms( $tkey ),
							'default' => '0',
							'condition' => [
		...
		$this->add_control(
				...
				[
					...
					. '<br>' . __( 'In the POST page will show all Terms associated to the current post.', 'dynamic-content-for-elementor' )
					. '<br>' . __( 'In the TERM page will show all its Terms children.', 'dynamic-content-for-elementor' ),
		...
		$this->add_control(
				'show_childlist_depth',
				[
					'label' => __( 'Max Child Depth', 'dynamic-content-for-elementor' ),
		...
		$this->add_control(
				'show_count',
				[
					'label' => __( 'Show Count', 'dynamic-content-for-elementor' ),
					'type' => Controls_Manager::SWITCHER,
				]
		);
		...
		$this->add_control(
				'show_separators_custom',
				[
					'label' => __( 'Custom Separator', 'dynamic-content-for-elementor' ),
					'type' => Controls_Manager::TEXT,
					'condition' => [
						'menu_style' => 'horizontal',
						'show_separators' => 'hidden',
					],
				]
		);
		...
		if ( Helper::is_acf_active() ) {
			...
			$this->add_responsive_control(
					'space', [
						...
						'selectors' => [
							'{{WRAPPER}} .dce-menu li img' => 'width: {{SIZE}}{{UNIT}};',
						],
						'condition' => [
							'image_acf_enable!' => '',
						],
		...
		$this->add_control(
				...
				[
					...
					'selectors' => [
						...
						'{{WRAPPER}} .dce-menu.horizontal li .dce-term-separator' => 'color: {{VALUE}};',
					],
				]
		);
		$this->add_responsive_control(
		...
		$this->start_controls_tab('dce_image_normal',
				[
		...
		$this->add_control(
				...
				[
					...
					'selectors' => [
						'{{WRAPPER}} .dce-menu li img' => 'opacity: {{SIZE}};',
					],
				]
		);
		...
		$this->add_group_control(
				...
				[
					'name' => 'dce_image_css_filters',
					'selector' => '{{WRAPPER}} .dce-menu li img',
				]
		);

		$this->end_controls_tab();

		$this->start_controls_tab('dce_image_hover',
				[
		...
		$this->add_control(
				...
				[
					...
					'selectors' => [
						'{{WRAPPER}} .dce-menu li a:hover img' => 'opacity: {{SIZE}};',
					],
				]
		);
		...
		$this->add_group_control(
				Group_Control_Css_Filter::get_type(),
				[
					'name' => 'dce_image_css_filters_hover',
					'selector' => '{{WRAPPER}} .dce-menu li a:hover img',
				]
		);

		$this->add_control(
				'dce_image_background_hover_transition',
				[
					...
					'selectors' => [
						'{{WRAPPER}} .dce-menu li img' => 'transition-duration: {{SIZE}}s',
					],
				]
		);
		...
		$this->add_responsive_control(
				'dce_image_border_radius',
				[
					'label' => __( 'Border Radius', 'dynamic-content-for-elementor' ),
					'type' => Controls_Manager::DIMENSIONS,
					'size_units' => [ 'px', '%' ],
					'selectors' => [
						'{{WRAPPER}} .dce-menu li img' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
					],
				]
		);
		$this->add_group_control(
		...
		$this->add_responsive_control(
				'dce_image_padding', [
					...
					'type' => Controls_Manager::DIMENSIONS,
					'selectors' => [
						'{{WRAPPER}} .dce-menu li img' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
					],
				]
		);
		...
		if ( ! empty( $terms ) && ! is_wp_error( $terms ) ) {
			...
			foreach ( $terms as $tkey => $term ) {
				...
				if ( isset( $settings['image_acf_enable'] ) && $settings['image_acf_enable'] ) {
					...
					$imageField = get_term_meta( $term->term_id, $idFields, true );
					$typeField = '';
					if ( $imageField ) {
						$imageSrc = false;
