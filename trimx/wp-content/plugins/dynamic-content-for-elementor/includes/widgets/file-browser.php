...
class DCE_Widget_FileBrowser extends DCE_Widget_Prototype {
	...
	protected function register_controls_content() {
		...
		$this->add_control(
				...
				[
					...
					'default' => date( 'Y' ),
					'description' => __( 'You can add more files trought MediaLibrary or via FTP', 'dynamic-content-for-elementor' ),
					'condition' => [
		...
		if ( ! empty( $taxonomies ) ) {
			foreach ( $taxonomies as $tkey => $atax ) {
				if ( $tkey ) {
					$terms = Helper::get_taxonomy_terms( $tkey, true );
					$this->add_control(
						'terms_' . $tkey, [
		...
		$this->add_control(
				'private_access', [
					'label' => __( 'Set Private access', 'dynamic-content-for-elementor' ),
					'type' => Controls_Manager::SWITCHER,
					'separator' => 'before',
					'description' => __( 'WARNING: direct access will be disabled for ALL files in folder and subfolder', 'dynamic-content-for-elementor' ),
					'condition' => [
						'path_selection' => [ 'custom', 'uploads' ],
					],
				]
		);
		$this->add_control(
		...
		$this->add_control(
					'user_redirect',
					[
						'label' => __( 'Redirect', 'dynamic-content-for-elementor' ),
						'type' => Controls_Manager::URL,
						'placeholder' => __( '/subscribe-plan-page', 'dynamic-content-for-elementor' ),
						'description' => __( 'Redirect Unauthorized users', 'dynamic-content-for-elementor' ),
						'condition' => [
							'private_access!' => '',
							'path_selection' => [ 'custom', 'uploads' ],
						],
		...
		$this->add_control(
				'resized', [
					...
					'type' => Controls_Manager::SWITCHER,
					'description' => __( 'WordPress automatically create for every uploaded image many resized version (e.g.:my-image-150x150.png, another-img-310x250.jpg), if you want to view them, enable this setting', 'dynamic-content-for-elementor' ),
					'condition' => [
						'path_selection' => [ 'uploads', 'custom' ],
					],
				]
		...
		$this->add_control(
				'img_icon', [
					...
					'type' => Controls_Manager::SWITCHER,
					'description' => __( 'If the file is an image then use it\'s thumb as icon', 'dynamic-content-for-elementor' ),
					'condition' => [
						'path_selection' => 'media',
					],
				]
		...
		$this->add_control(
				'search', [
					'label' => __( 'Quick search form', 'dynamic-content-for-elementor' ),
					'type' => Controls_Manager::SWITCHER,
					'separator' => 'before',
				]
		);
		...
		$this->add_control(
				'enable_metadata', [
					'separator' => 'before',
					'label' => __( 'Metadata info', 'dynamic-content-for-elementor' ),
					'type' => Controls_Manager::SWITCHER,
				]
		);
		...
		$this->start_controls_section(
			...
			$this->add_control(
				...
				[
					...
					'type' => Controls_Manager::TEXT,
					'default' => __( '* at least 3 characters', 'dynamic-content-for-elementor' ),
					'placeholder' => __( '* at least 3 characters', 'dynamic-content-for-elementor' ),
				]
			);
			$this->add_control(
					'search_quick',
		...
		$this->add_control(
				'enable_metadata_hide', [
					...
					'type' => Controls_Manager::SWITCHER,
					'description' => __( 'You can select what file and folder to hide if you don’t want to share it', 'dynamic-content-for-elementor' ),
					'condition' => [
						'enable_metadata' => 'yes',
					],
				]
				...
				if ( ! Helper::is_empty_dir( $fulldir ) || $settings['empty'] ) {
					if ( ! $hide || \Elementor\Plugin::$instance->editor->is_edit_mode() ) {
						...
						<?php echo $hideHtml; ?>
							...
							<?php if ( $customTitle ) { ?>
									</a> <input type="text" class="dce-dir-title" data-dir="<?php echo $kdir; ?>" name="dce-dir-browser[<?php echo $kdir; ?>][title]" value="<?php echo $title; ?>" /> <a class="inline-block" href="<?php echo Helper::path_to_url( $fulldir ); ?>" target="_blank">
							<?php } else { ?>
	...
	public function displayFileSearch( $settings ) {
		...
		<?php if ( $settings['search_text'] != '' ) {
			...
			<?php if ( $settings['search_notice'] ) {
				?><div class="dce-search-desc"><small><?php echo $settings['search_notice']; ?></small></div><?php } ?>
			<?php if ( ! $settings['search_quick'] ) {
				?><div class="text-right dce-search-buttons"><?php if ( $settings['search_reset'] ) {
	?><input class="reset" type="reset" value="<?php echo $settings['search_reset_text']; ?>""> <?php } ?><input class="find" type="submit" value="<?php echo $settings['search_find_text']; ?>"></div><?php } ?>
		</form>
		<br />
		<?php
	}
