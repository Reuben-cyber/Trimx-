	...
	public static function show_license_form() { ?>
		...
		if ( isset( $_POST['licence_key'] ) ) {
			...
			if ( DCE_LICENSE != $licence_key || ! intval( $_POST['licence_status'] ) ) {
				...
				$res = LicenseSystem::call_api( 'activate', $licence_key, true );
		...
		$license_data = LicenseSystem::call_api( 'status-check', $licence_key, $licence_check, true );
		if ( $license_data ) {
		...
		<div class="dce-notice <?php echo $classes; ?>">
			<h2><?php echo esc_html( __( 'License Status', 'dynamic-content-for-elementor' ) ); ?> <a href="?<?php echo Helper::recursive_sanitize_text_field( $_SERVER['QUERY_STRING'] ); ?>&licence_check=1"><span class="dashicons dashicons-info"></span></a></h2> 
			...
			<?php ( $licence_status ) ? submit_button( 'Deactivate', 'cancel' ) : submit_button( __( 'Save Key and Activate', 'dynamic-content-for-elementor' ) ); ?>
			</form>
		<?php
		if ( $licence_status ) {
			if ( $dce_domain && $dce_domain != DCE_INSTANCE ) {
				...
				<?php } else { ?>
					...
					<p><?php _e( 'Thank you for using our plugin.', 'dynamic-content-for-elementor' ); ?><br><?php _e( 'Feel free to create your new dynamic and creative website.', 'dynamic-content-for-elementor' ); ?><br><?php _e( 'If you like our plugin then please recommend it to your friends.', 'dynamic-content-for-elementor' ); ?></p>
				<?php
				}
		} else {
			?>
				<p><?php _e( 'Enter your license here to keep the plugin updated, obtaining new features, future compatibility, more stability and security.', 'dynamic-content-for-elementor' ); ?></p>
				<p><?php _e( 'You still don’t have one?', 'dynamic-content-for-elementor' ); ?> <a href="https://www.dynamic.ooo" class="button button-small" target="_blank"><?php _e( 'Get it now!', 'dynamic-content-for-elementor' ); ?></a></p>
		<?php } ?>
		</div>
		...
		if ( $licence_status ) {
			...
			$backups = glob( DCE_BACKUP_PATH . '/dynamic-content-for-elementor_*.zip' );
			if ( ! empty( $backups ) ) {
