...
abstract class Skin_Base extends Elementor_Skin_Base {
	...
	public function render() {
		...
		if ( $this->parent->get_settings( 'infiniteScroll_enable' ) && \Elementor\Plugin::$instance->editor->is_edit_mode() ) {
			Helper::notice( '', __( 'Infinite Scroll is not seen correctly in the Elementor editor due to technical limitations but works correctly in the frontend.', 'dynamic-content-for-elementor' ) );
		}
	...
	protected function render_custommeta() {
		...
		if ( is_array( $custommeta_items ) && ! empty( $custommeta_items[0]['metafield_key'] ) ) {
			...
			foreach ( $custommeta_items as $key => $item ) {
				...
				$metafield_button_label = wp_kses_post( $item['metafield_button_label'] );
