...
class Assets {
	...
	private static function init_vendor_js() {
		...
		self::$vendor_js = [
			...
			'dce-tippy' => [
				'path' => 'https://unpkg.com/tippy.js@6',
				'deps' => [ 'dce-popper' ],
			],
		];
	...
	public static $scripts = array(
		...
		'dce-js-field' => '/assets/js/js-field.js',
		...
		'dce-dynamicPosts-grid-filters' => '/assets/js/dynamic-posts-v2-skin-grid-filters.js',
	...
	public function loader_tag_filter( $tag, $handle, $src ) {
		...
		if ( ! in_array( $handle, $modules_script, true ) ) {
			return $tag;
		}
	...
	public static function dce_globals_stylescript() {
		...
		if ( get_option( 'enable_smoothtransition' ) || $is_in_editor ) {
			...
			wp_enqueue_style( 'animsition-base', DCE_URL . 'assets/lib/animsition/css/animsition.css', array(), DCE_VERSION, true );
	...
	public static function register_dce_scripts() {
		foreach ( self::$scripts as $name => $path ) {
			...
			wp_register_script( $name, $path, $deps, DCE_VERSION, true );
		}
	}
	...
	public static function register_vendor_scripts() {
		foreach ( self::$vendor_js as $name => $path ) {
			...
			if ( substr( $path, 0, 4 ) != 'http' ) {
				...
				wp_register_script( $name, $path, $deps, null, true );
			}
		}
	}
	...
	public function enqueue_admin_styles() {
		...
		wp_enqueue_script( 'dce-admin-js', DCE_URL . 'assets/js/admin.js', [], DCE_VERSION, true );
	}
	...
	public function dce_editor() {
		...
		wp_localize_script( 'dce-script-editor', 'posts_v2_item_label_localization', [
			'item_title' => '<i class="fa fa-font" aria-hidden="true"></i> ' . __( 'Title', 'dynamic-content-for-elementor' ),
			'item_image' => '<i class="fa fa-picture-o" aria-hidden="true"></i> ' . __( 'Featured Image', 'dynamic-content-for-elementor' ),
			'item_date' => '<i class="fa fa-calendar" aria-hidden="true"></i> ' . __( 'Date', 'dynamic-content-for-elementor' ),
			'item_termstaxonomy' => '<i class="fa fa-folder-open-o" aria-hidden="true"></i> ' . __( 'Terms', 'dynamic-content-for-elementor' ),
			'item_content' => '<i class="fa fa-align-left" aria-hidden="true"></i> ' . __( 'Content', 'dynamic-content-for-elementor' ),
			'item_author' => '<i class="fa fa-user-circle-o" aria-hidden="true"></i> ' . __( 'Author', 'dynamic-content-for-elementor' ),
			'item_custommeta' => '<i class="fa fa-check-circle-o" aria-hidden="true"></i> ' . __( 'Custom Meta Fields', 'dynamic-content-for-elementor' ),
			'item_readmore' => '<i class="fa fa-hand-pointer-o" aria-hidden="true"></i> ' . __( 'Read More', 'dynamic-content-for-elementor' ),
			'item_posttype' => '<i class="fa fa-thumb-tack" aria-hidden="true"></i> ' . __( 'Post Type', 'dynamic-content-for-elementor' ),
		]);
