...
trait Trait_WP {
	...
	public static function get_types_registered() {
		...
		$type_esclusi = TemplateSystem::$supported_types;
	...
	public static function get_term_value( $term = null, $field = 'name', $single = null ) {
		...
		if ( $termValue === null ) { // for meta created with Toolset plugin
	...
	public static function get_dynamic_value( $value, $fields = array(), $var = 'form' ) {
		...
		if ( is_string( $value ) ) {
			$value = Tokens::do_tokens( $value );
			if ( ! Tokens::$data ) {
				$value = do_shortcode( $value );
				if ( ! empty( $fields ) ) {
					$value = self::replace_setting_shortcodes( $value, $fields );
					$value = Tokens::replace_var_tokens( $value, $var, $fields );
				}
			}
		}
		return $value;
