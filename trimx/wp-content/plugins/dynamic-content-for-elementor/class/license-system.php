...
class LicenseSystem {
	...
	public function activation_advisor() {
		...
		$tab_license = ( isset( $_GET['page'] ) && $_GET['page'] == 'dce-license' ) ? true : false;
	...
	public static function dce_expired_license_notice() {
		...
		if ( $dce_expiration_date && current_user_can( 'administrator' ) ) {
			if ( $dce_expiration_date < gmdate( 'Y-m-d' ) ) {
				Notice::dce_admin_notice__danger( sprintf( __( 'Your License Expired on ' . $dce_expiration_date . '. Please renew your license or you can\'t get more plugin updates. %1$sExtend your license now%2$s', 'dynamic-content-for-elementor' ), '<a class="btn button" target="_blank" href="https://www.dynamic.ooo">', '</a>' ) );
				return false;
			}
		}
		return true;
